{
  ...
  u-boot ? (import ../uboot { inherit pkgs; })
}:
let
...
stdenv.mkDerivation {
  ...
  buildPhase = ''
    ...
    # blx_fix: pad bl30/bl2 to fixed sizes, concatenate with
    # associated data (bl301 / acs) to produce _new.bin files.
    ...
    # Signing pipeline via aml_encrypt_g12b (pre-built x86_64 binary)
